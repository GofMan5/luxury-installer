...
- Manifest schema 6 adds declarative host requirements: `[install.requires]` carries one target-scoped `windows_minimum_version` predicate, evaluated read-only during preflight so an unmet requirement stops the install before anything is written and reports both the required and the detected version. Studio authors it, `luxury inspect` prints it, and JSONL v5 carries it in both directions; a Windows predicate on a non-Windows target is refused outright.
- Manifest schema 5 and JSONL v5 add one authenticated product identity: target-native payload icon plus HTTPS homepage/support. Bundle opening fully decodes bounded ICO/PNG/ICNS bytes, receipt v7 persists the snapshot, and same-version repair rejects metadata drift while Setup/Studio branding stays separate.
