...
- Linux launch never reopens the receipt-owned entrypoint pathname after hash/mode verification. User and system adapters duplicate that exact FD, execute its kernel-owned `/proc/self/fd/N` name, drop system credentials when required, and clear CLOEXEC only in the child for interpreter compatibility. Keep the inherited descriptor read-only and never fall back to the original package path. Windows launch must open every absolute parent component as a non-reparse directory without delete sharing, retain the complete chain plus final-file guards through direct spawn or `CreateProcessWithTokenW`, and reject path-only rechecks. macOS `launchctl asuser` remains pathname-bound.
