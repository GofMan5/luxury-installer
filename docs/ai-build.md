...
`init` creates only its exact generated `luxury.toml` and starter payload. A repeated init requires byte-identical content and never overwrites user changes. Studio's first successful real import removes that starter only when it remains the exact sole template file. For repeat releases, Studio can replace the complete payload with the contents of one selected directory: Rust copies and validates the new tree before swapping it, restores the old tree on ordinary validation/config failure, retains the staging backup if rollback itself fails, replaces executable intent, and clears an entrypoint that no longer exists with exact case together with shortcut intent that can no longer be bound. `luxury.toml` is untrusted and capped at `1 MiB` before parsing; optional `package.description` is bounded plain text in the core spec even for hand-edited projects.
...
- Studio lifts dirty state before the next user action, disables create/open/reload, and exposes one baseline-only undo. For close/Alt+F4, Rust emits a strict correlated `luxury://studio-close-query`; the renderer replies through exact `respond_studio_close` with only `{requestId, dirty}`. A missing, malformed, stale, or timed-out reply is treated as dirty, and only Rust opens the native discard confirmation. Do not add renderer dialog permission or a browser `beforeunload` prompt.
...
- Schema 4 `[install.shortcuts]` carries only `application_menu` and `desktop`. The target is always the authenticated exact entrypoint. Studio edits the two booleans; JSONL and the strict Setup review contract carry them. Current Setup bootstrap deliberately stops at typed `unsupported` preflight before rendering that review, until native transactional mutation is implemented.
...
- Cancellation targets one active cancellable operation ID. In Setup, a failed Tauri cancel invocation leaves install/uninstall active, reports the bounded public error inline, and re-enables the pathless cancel action for retry; only a correlated backend terminal event ends the operation. Successful launch is not rollbackable and is not cancellable.
- Linux launch opens the install root descriptor-relative with no-follow `openat`, opens and verifies the receipt entrypoint beneath that same FD, duplicates both guards, `fchdir`s to the retained root and executes the kernel-owned `/proc/self/fd/N` name. System launch drops credentials before `fchdir`; only the read-only executable FD clears CLOEXEC for interpreter compatibility. Never reintroduce `Command::current_dir`, reopen either pathname, or add a fallback. Windows launch must open every absolute parent component as a non-reparse directory without delete sharing, retain the complete chain plus final-file guards through direct spawn or `CreateProcessWithTokenW`, and reject path-only rechecks. macOS uses the same no-follow root/file verification and retained-root `fchdir`; system credentials drop first and `launchctl asuser` inherits that cwd. Keep the remaining macOS executable pathname ceiling explicit.
