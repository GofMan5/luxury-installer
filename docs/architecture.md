...
Windows pathname/reparse parent binding and general create/delete directory durability remain incomplete. Linux receipt-owned launch is descriptor-bound, but other Unix source-leaf mutations and macOS launch remain pathname-bound. Mapped writers, working-directory retargeting and hostile same-user namespace races remain. Native macOS/APFS power-cut behavior is not proven. Process containment is not a sandbox. Signed native containers, platform signing/notarization, and published native recovery/signature matrices remain release blockers.
