...
Windows system authorization transfers file authority, not a path. The Tauri shell pins a non-reparse single-link package handle; after kernel PID and Authenticode checks the helper duplicates it from the authenticated parent, validates the exact package/action/host/system scope, and derives Known Folder roots itself. The same authenticated helper runs `prepare_system_install` during system bootstrap/retry and again after a successful install/uninstall before its terminal frame, so maintenance state comes from the protected receipt without another UAC prompt. Tauri validates that state before caching it or exposing the authoritative review with the completion event. Missing or invalid terminal preparation clears the cached selection and the next bootstrap performs a new privileged refresh. The engine exposes explicit system constructors for the privileged composition root; existing constructors remain user-only, so ordinary CLI/stdio cannot reinterpret system receipts.
...
- Schema v4 adds bounded `install.shortcuts` intent: application-menu and desktop booleans only. Both are bound to the existing exact receipt-owned entrypoint; packages cannot name a different target, arguments, working directory, URL, or environment. The compiler, engine receipt, JSONL, Studio and Setup understand the intent. Windows `.lnk` and Linux `.desktop` codecs exist, but native mutation remains fail-closed as `unsupported` until WAL v5 implements transactional external-root creation/rollback; macOS also requires a real signed product `.app`.
...
- Ownership receipt v6 stores payload signer, authorized publisher, optional entrypoint, shortcut intent, authenticated display name and the exact typed native artifacts (location, leaf, size, mode and SHA-256). Receipt v1-v5 remain readable without background migration; v5 intent carries no artifact authority and cannot be used to adopt or remove an external shortcut.
...
Package and destination locks use a fixed order. Platform envelope v2, ownership receipt v6, and journal v4 bind state to package/destination/install-base identity and exact install scope; legacy journal v2/v3 is user-only. Journal v4 still cannot authorize external shortcut mutation: the next prerequisite is WAL v5 with typed shortcut locations and re-resolved root identities. Install and state roots may live on different filesystems: payload publication occurs on the install volume and receipt cutover on the state volume.
