...
- Ready: show the current project, manifest facts, payload counts/size, target, one primary build action, and secondary native-folder/reload actions in the rail. A clean compatible form says **Build**; a dirty valid compatible form says **Save and build**, awaits the existing Rust save/revalidation, and starts native build only after success. The exact local target comes from Rust. When project target and local target differ, disable build and show both targets plus the matching-runner/Native project build route. The Rust-owned dialog suggests a bounded product-name artifact rather than a technical package-ID filename.
