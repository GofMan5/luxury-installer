...
| GUI authoring plus automation | **Implemented**: Studio, human CLI, typed JSONL v5, AI skill | Advanced Installer/InstallBuilder lead in GUI; Inno/NSIS/WiX lead in text automation | Keep both surfaces synchronized; add a shared multi-target workspace only after identity/version drift proves the need. |
...
| Install conditions and OS/runtime requirements | **Partial**: exact target/architecture, scope, space and permission checks, plus schema 6 `[install.requires]` with one target-scoped minimum-Windows predicate that fails closed in read-only preflight | Mature tools expose OS versions, RAM, runtime and custom conditions | **P1.** Bounded declarative predicates with actionable preflight output. |
...
| 17 | partial | Global prerequisite predicates and declarative conditions | Existing manifest/host preparation | Spec/engine/CLI tests | Manifest schema 6 `[install.requires]` ships one target-scoped `windows_minimum_version` predicate evaluated read-only in preflight; further predicates and the non-Windows equivalents remain open. |
