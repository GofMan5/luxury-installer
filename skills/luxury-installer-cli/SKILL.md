...
- Treat manifest schema 4 `install.shortcuts` as two booleans only: `applicationMenu`/`application_menu` and `desktop`. Either requires the existing exact entrypoint; never add target, args, cwd, URL, shell, or environment fields. Current native preparation intentionally returns `unsupported` before mutation until platform adapters ship.
