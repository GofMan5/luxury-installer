...
description: Build native Luxury Installer outputs, inspect and sign internal packages, automate install/update/repair/uninstall/launch, and use the strict JSONL v5 protocol. Use for Luxury Installer repository work, AI-driven authoring, CI, Studio backend operations, or any request involving the luxury command or native packager.
---
...
- Treat schema 6 `install.requires` as read-only host policy: today only `windowsMinimumVersion` (`major.minor.build`), accepted solely for a Windows target, evaluated in preflight, and failing closed as `unsupported` before any mutation. Never retry an unmet requirement.
