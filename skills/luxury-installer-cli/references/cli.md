...
On Linux, the output argument is a new directory containing `.deb` and `.rpm`; on macOS it is a new `.dmg`. The packager creates and removes the internal `.luxpkg` itself. Studio supervises the complete native packager descendant tree through `luxury-process`, so cancel, timeout, and primary exit terminate/reap child tools before returning, then removes the exact parent-owned temporary work directory without scanning neighboring names. Use low-level `luxury build` only for signed-package, protocol, or lifecycle automation that explicitly needs that trust boundary.
